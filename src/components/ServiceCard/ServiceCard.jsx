...
const ServiceCard = ({ icon, title, description, path, content, status }) => {
  return (
    <div className="bg-sky group text-center  my-10 p-10 mx-4 relative max-w-sm mx-auto ">
      <div>
        <FaDumbbell className={`mx-auto text-[50px] text-gray  group-hover:text-orange my-4 ${status ? 'text-orange' : ''}`} />
      </div>
      <div className="my-6">
      ...
      <div className={`absolute bg-red w-full left-0 bottom-0 pt-4 group-hover:opacity-100 opacity-0 transition-opacity duration-300 ${status ? 'opacity-100' : ''}`}>
